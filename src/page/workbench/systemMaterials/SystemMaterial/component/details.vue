<template>
  <div class="systemMaterialsDetailsUser">
    <el-dialog
      :title="$t('详情')"
    ...
    >
      <ul>
        ...
        >
          <p>{{ $t(item.label) }}</p>
          <p>   {{ ((formData[item.value]) +((item.unit?item.unit:''))) }}</p>
        </li>
      </ul>
      <el-button class="cursor" type="primary" @click="handleClose">{{ $t('确认') }}</el-button>
    </el-dialog>
  </div>
</template>
<script>
