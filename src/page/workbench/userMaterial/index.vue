    ...
    <div class="contents">
      ...
      <div v-if="idx===0" class="mainRight">
        ...
        <!-- <el-select
          ...
          clearable
          class="first"
          @change="watchList($event,1)"
        >
          <el-option v-for="(item,index) in modeList" :key="index" :label="item.label" :value="item.value" />
        </el-select> -->
...
export default {
  ...
  methods: {
    ...
    edit(row) {
      ...
      //    this.formDatas.m = `[${this.formData.m},${this.formData.n}]`
      this.addShow = true
      this.editState = 1
    },
    saves(obj) {
      ...
      _api.update(obj).then((res) => {
        this.$message.success(res.message)
        this.$refs.systemMaterials.getData(this.$refs.systemMaterials.pageVo.currentPage, this.$refs.systemMaterials.pageVo.pageSize, { searchWord: this.modeVal })
        this.addShow = false
        this.formDatas = {}
      }, () => {
        this.formDatas = this.initialObj
